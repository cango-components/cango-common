<template>
	<div class = "cg-uploadify__base" >
		<div  v-if="fileList" class = "cg-uploadify__main">
			<div v-for="file in fileList"  @click='remove(file)' class = "cg-uploadify__file">
...
export default {
	...
	methods : {
		...
		reloadFile : function(){
			var self = this;
			if(!isBlank(self.value)){
				getFileList(self.value, function(list){
					let fileList = clone(self.fileList);
					self.fileList = []
					// 此处通过合并操作可以使用浏览器缓存，降低反复下载图片流量
					for(let i = 0; i < list.length; i++){
						let flg = false;
						for(let k = 0; k < fileList.length; k++){
							if((list[i]["filePath"] == fileList[k]["filePath"]) && !isBlank(fileList[k]["url"])){
		...
		onUpload  : function(e){
			if(e && e.target && e.target.files){
				...
				if(this.fileNum == 1){
					uploadFile([e.target.files[0]], [], func, this.prefix);
				}else{
					uploadFile(e.target.files, [], func, this.prefix);
				}
			}
		},
		...
		resizeValue : function(){
			...
			createGroupId(self.fileList, func);
		}
	},
	watch : {
