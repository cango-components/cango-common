<template>
  <div class = "cg-checkbox__main" v-bind:class="errorMsg != '' ? 'cg-checkbox__error' : ''" >
    <div class = "cg-checkbox__label">
      {{ label }}
    </div>
    <div v-for="item in list" @click="putValue(item)" v-bind:class="isChecked(item) ? 'cg-checkbox__checked' : ''" class = "cg-checkbox__content">
      {{ showRecordName(item) }} {{ isChecked(item) ? 'checked' : '' }}
    </div>
  </div>
</template>
  ...
  methods : {
    ...
    showRecordName : function(record){
      if(this.showFunc){
        ...
        return getRecordValue(record, this.showName);
      }
    }
  },
  watch : {
